...
def our_model():
    ...
    m_sub_out = Subtract(name="m_output_score")([m_s0_removed, m_s1_removed])
...
def train_our_model(model):
    ...
    model.compile(optimizer=Adam(lr=1e-4),
                  ...
                  metrics={'b_output_x': 'acc',
                           ...
                           'f_output_state': 'mse',
                           'm_output_score': 'mse'},
                  loss_weights=[1.0, 1.0, 1.0, 1.0, 1000.0, 0.0],
                  )
...
def test_our_model(model, metric_model):
    ...
    # model.load_weights('weights-i/2-I.999-0.28402096.hdf5', by_name=True)
    model.load_weights('weights-zs/T.985-0.51052483.hdf5', by_name=True)
    ...
    while rollout_times < 10:
        ...
        pickle.dump(array, open("zero-shot-score-" + str(rollout_times) + ".p", "wb"))
